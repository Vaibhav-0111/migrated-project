...
interface ChanceGardenStepHelperProps {
  step: number;
  onNextStep: () => void;
  onComplete: () => void;
}
...
const steps = [
  {
    ...
    description: "Look at each basket. Which color has MORE balls?",
    visual: "🧺 🔵🔵🔵🔵🔵 🟡🟡",
  },
  {
    title: "More = More Likely",
    description: "The color with MORE balls is MORE likely to come out!",
    visual: "🔵 > 🟡 = 🔵 more likely!",
  },
  {
    title: "Try Again!",
    description: "Tap the basket where your target color has the MOST balls.",
    visual: "👆 Choose wisely!",
  },
];

export function ChanceGardenStepHelper({ step, onNextStep, onComplete }: ChanceGardenStepHelperProps) {
    ...
    >
      <div className="bg-card/95 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-border/50 max-w-md w-full">
        ...
        <div className="bg-muted/50 rounded-xl p-4 mb-4 text-center">
          <span className="text-xl">{currentStep.visual}</span>
        </div>
